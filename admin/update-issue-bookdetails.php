                ...
                <div class="panel panel-info">
                    ...
                    <div class="panel-body">
                        ...
                        if($query->rowCount() > 0)
                        {
                            ...
                            {               
                                $issued_date = date('Y-m-d',strtotime($result->IssuesDate));
                                $current_date = time();

                                function dateDiff($issued_date, $current_date)
                                {
                                    $date1_ts = strtotime($issued_date);
                                    $date2_ts = $current_date;
                                    $diff = $date2_ts - $date1_ts;
                                    return round($diff / 86400);
                                }

                                $dateDiff = dateDiff($issued_date, $current_date);

                                function calculateFine($dateDiff)
                                {   
                                    if($dateDiff > 15)
                                    {
                                        return ($dateDiff-15);
                                ...
                                <?php echo htmlentities($dateDiff);?>
                                </div>

                                <div class="form-group">
                                <label>Fine (in INR) :</label>
                                <input class="form-control" type="text" name="fine" id="fine"  value=<?php echo htmlentities($fine); ?> readonly />
                                </div>
